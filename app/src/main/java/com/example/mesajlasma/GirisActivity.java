...
public class GirisActivity extends AppCompatActivity {
    ...
    public void ekle(final String kadi) {
        final Task<Void> voidTask = reference.child("Kullanıcılar").child(kadi).child("kullaniciadi").setValue(kadi).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    Toast.makeText(getApplicationContext(), "Başarılı ile giris yaptınız", Toast.LENGTH_LONG).show();
